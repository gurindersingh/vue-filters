...
export const append = (text, suffix = '...') => text + suffix;
...
export const substr = (text, length, clamp) => {
    clamp = clamp || '...';
    length = length || 30;
    return text.substr(0, length) + clamp;
};

export const strCount = (text, limit, clamp) => {
...
export const prettyBytes = (bytes, decimals, kib = false) => {
...
export const formatDate = (date, formatTo, $null = '---') => {
    if (!date) return $null;
...
// Money formating
export const moneyInDollars = (money, symbol = 'CAD$') => {
    money = (money / 100).toFixed(2);
    return `${symbol} ${money}`;
};

// Phone formating
export const phone = (phone, type = 'us') => {
